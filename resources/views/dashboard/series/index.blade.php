    ...
    <div class="block-header">
        <div class="row">
            <div class="col-lg-7 col-md-5 col-sm-12">
                <h2>All series
                    <small class="text-muted">Welcome to series</small>
                </h2>
            </div>
            <div class="col-lg-5 col-md-7 col-sm-12">
                @if(auth()->guard('admin')->user()->hasPermission('create_series'))
                <a href="{{route('dashboard.series.create')}}">
                ...
                <ul class="breadcrumb float-md-right">
                    <li class="breadcrumb-item"><a href="{{url('dashboard')}}"><i class="zmdi zmdi-home"></i> series</a>
                    </li>
        ...
        <div class="row clearfix">
            <div class="col-md-12">
                <div class="card patients-list">
                    <div class="header">
                        <h2><strong>series </strong><span>({{$series->total()}})</span></h2>
                    </div>
                    <div class="body">

                        <form action="{{ route('dashboard.series.index') }}" method="GET">
                            <div class="row clearfix">
                                <div class="col-5">
                                ...
                                <div class="col-3">
                                    <select name="category" class="form-control z-index show-tick"
                                ...
                                <div class="col-3">
                                    <select name="actor" class="form-control z-index show-tick" data-live-search="true">
                                        ...
                                        <option value="{{$actor->id}}"
                                            {{request()->actor == $actor->id ? 'selected' : ''}}>{{$actor->name}}
                                        </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <table class="table m-b-0 table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><span class="list-name">{{$serie->name}}</span></td>
                                            <td>{{$serie->year}}</td>
                                            <td>{{$serie->seasons}}</td>
                                            <td>
                                                <i class="zmdi zmdi-star"></i> {{$serie->ratings->avg('rating')}}
                                                <a href="{{route('dashboard.ratings.index', ['serie' => $serie->id])}}"><small
                                                        style="font-size: 10px">({{$serie->ratings->count()}}
                                                        votes)</small></a>
                                            </td>
                                            <td>
                                                <button title="show overview" value="{{$serie->overview}}"
                                            ...
                                            <td>
                                                ...
                                                <a href="{{ route('dashboard.actors.index', ['serie' => $serie->id]) }}"
                                                    class="btn btn-info btn-sm">Actors</a>
                                                @else
                                                <button class="btn btn-info btn-sm disabled"
                                                    style="cursor: no-drop">series</button>
                                                @endif
                                            </td>
                                            <td>
                                                ...
                                                @if(auth()->guard('admin')->user()->hasPermission('delete_series'))
                                                <form action="{{ route('dashboard.series.destroy', $serie) }}"
